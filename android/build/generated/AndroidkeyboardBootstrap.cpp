...
#define TAG "con.familydollar.androidkeyboard"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Androidkeyboard_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Androidkeyboard.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::AndroidkeyboardBindings::lookupGeneratedInit(
...
static void Androidkeyboard_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Androidkeyboard_getBinding)->GetFunction());
}

static void Androidkeyboard_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::AndroidkeyboardBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry AndroidkeyboardBinding = {
	"con.familydollar.androidkeyboard",
	Androidkeyboard_init,
	Androidkeyboard_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_con_familydollar_androidkeyboard_AndroidkeyboardBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("con.familydollar.androidkeyboard", &AndroidkeyboardBinding);
	titanium::KrollBindings::addExternalLookup(&(::AndroidkeyboardBindings::lookupGeneratedInit));
}
